...
為了保持根目錄的整潔與專案架構的清晰，本專案已完成檔案整理。所有的過期測試腳本、寫死絕對路徑的舊處理檔案，均已移入 `tmp/` 資料夾中。以下是整理後的最新專案結構：
...
├── tmp/                   # 臨時與過期檔案暫存區 (包含 scratch_analyze.py 等無用檔案與資料夾)
...
1. **移除檔案**：
   ...
   * *(推薦備份做法)* 將不需要的 CSV 移至根目錄的 `archive/` 或 `tmp/` 中。由於 Dashboard 與編譯器只會掃描 `results/`，移出此目錄的檔案將不會被載入。
