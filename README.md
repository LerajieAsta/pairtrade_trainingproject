...
>
> **特徵維度**（`analysis/prop1_feature_dimension.py`，初步）：自既有 SEC companyfacts
> 快取解出 40 個 Green et al. 特徵（**零網路成本**——companyfacts 一次回傳全部概念），
> 10 個通過 70% 覆蓋率門檻（以 PIT 成分股身分為分母），連續維度 **7 → 17**。
> 2012+ 逐步鏈經 BH-FDR **無一顯著**，且兩大效果幾乎抵消（全域插補 −0.716pp、
> +10 特徵 +0.685pp）；對 GICS 為 +0.253pp（$p$=0.647），**未優於** 7 維版的 +0.455pp。
> ⚠️ 估值比率（bm/ep/cfp/sp/dy/lev）尚未納入，補齊後將重跑（見「資料範圍與已知限制」）。
>
...
> 到 75%（不分組）**，涵蓋 one-hot 權重、插補方式、特徵集、分組方式四個維度，
> 而 2012+ 績效全部落在 **−1.9% ~ −0.9%、彼此無顯著差異**。
...
本研究自 SEC XBRL companyfacts 建出 40 個（PIT 對齊於申報日），僅 **10 個**通過
70% 覆蓋率門檻（分母為 PIT 成分股身分），連續特徵維度 7 → 17。三層限制：

- **時間**：XBRL 強制申報自 ~2009 起，2000–2008 完全無財報特徵，故該系列實驗
  的分析限制在 2012 年後（覆蓋率穩定期）
- **估值比率缺席**：`bm`/`ep`/`cfp`/`sp`/`dy`/`lev` 需市值 → 需未調整股價，
  而 Tiingo 免費方案上限為**每月 500 個不重複代號**；774 檔成分股中已取得 495 檔
