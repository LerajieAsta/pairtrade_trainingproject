...
> 效果量與舊版幾乎相同（年化 +0.719% vs 舊 0.732pp）——偽重複影響的是**推論正當性**，
> 不是效果量本身。
>
...
>
> **命題2 成立（相對宣稱）**——DRL vs Z-Score，五種配對底逐日差分 bootstrap 全部顯著
> （p = 0.0000–0.0042），五個 95% CI 完全落在零的右側（最保守下界 +0.22pp）。
...
> **BH-FDR 校正後無一組達統計顯著**（校正後最小 p = 0.377；校正前最小 p = 0.058）。
...
> +12 特徵 −0.482pp，淨值 −0.001pp）；對 GICS 為 +0.283pp（$p$=0.613），
> **未優於** 7 維版的 +0.455pp。
>
...
>
> **綜合——貫穿命題 1 的核心發現**：跨產業配對比例在六種設定下自 **0%（GICS）
> 到 69%（不分組）**，涵蓋 one-hot 權重、插補方式、特徵集、分組方式四個維度，
> 而 2012+ 績效全部落在 **−1.8% ~ −0.7%、彼此無顯著差異**
> （跨產業比例與報酬統一以 2012+ 為基準；先前的 75% 係全期數字，基準不一致）。
> **「選哪些配對」在極大範圍內不影響報酬**——此結論比「ML 分群無效」更強，
> 指向報酬由配對選擇以外的因素決定（交易成本、市場效率、或此策略類別的報酬衰減）。
> 形成期實作差異、交易機制、特徵維度**三者皆已檢驗且皆非命題 1 失敗的原因**；
> 剩餘差距歸因於**資料可得性限制**而非設計選擇。
>
...
> 不再需要事後校正。六組配置（3 配對底 × 2 交易端）年化落在 −0.44% 至 +0.61%，
> bootstrap 檢定**無一顯著**（p = 0.40–0.82），CI 寬達 2–3.5pp。
> 等權組合的年化 Sharpe 僅 0.05–0.18，欲達統計顯著約需 123 年以上樣本——
> 此為結構性限制而非樣本不足。
> Deflated Sharpe 降為附錄：僅在改報「網格最佳格」時才需要（SR0 = 0.415、
> 最高 SR 0.343，全數不通過；N 由 `_trial_specs` 執行時實地清點相異 METHOD 數）。
> **命題2 是「A 優於 B」的相對宣稱，不主張策略本身可獲利。**
>
