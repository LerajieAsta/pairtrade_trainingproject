...
> **研究結論摘要**（更新於 2026-09-08，對沖口徑修正 + 全鏈重算後）
>
> ⚠️ **本區塊只放結論方向，不放完整數字。**
> 論文引用的每一個數字以 `results/analysis/thesis_numbers.md`（由
> `tools/thesis_numbers.py` 自資料庫重算）與 `thesis/04_實證結果.md` 為準。
> 本 README 曾長期維護第二份完整數字，結果是兩處各自老化、彼此矛盾
> （2026-09-08 發現此處仍停在 2026-08-11 的 $N$ = 110、
> 「組合系統六組中五組顯著」等已被推翻的敘述）。故改為只留方向與指標。
>
> **統計基礎。** 抽樣單位為**時間**而非「15 個參數格」——後者共用同一份資料、
> 同一段期間、同一批配對，屬偽重複。主檢定為逐日報酬差 + 循環 block bootstrap
> （$L$ = 126 ＝一個完整交易期，10,000 次重抽），Newey-West HAC 並行計算為對照欄。
> 樣本 6,287 個交易日，比較口徑為 15 個參數配置的**等權組合**。
>
> | 檢定 | 內容 | 方向 | BH 校正後顯著 |
> | :--- | :--- | :--- | :---: |
> | 分組層（命題 1） | 資料驅動分群 vs GICS 產業分組 | 9/9 偏向 GICS | 0/9 |
> | 交易層（命題 2） | DL-THR vs 固定門檻 Z-Score | 5/5 為正 | **3/5** |
> | 兩層組合 | 完整系統 vs 傳統基準 | 全期 2/3 為負 | 0/3 |
>
> **交易層是全篇唯一通過多重檢定校正的一組**，且該位移已由預先註冊的判準確認
> 超出重訓雜訊（120 格中 88 格，$p$ = 3.7e−36；`dev/drl_hedge/`）。
> 惟其中兩組的校正後 $p$ 恰為 0.050。
>
> **通過校正不等於可交易。** 同一批策略的**絕對**報酬檢定 **6/6 不顯著**，
> Deflated Sharpe（$N$ = 53，$SR_0$ = 0.408）**6/6 未達 0.95**，
> 成本餘裕在 K-means 底為負。三者任一都足以否定「可交易」的宣稱。
> **本專案不主張任何配置具可交易的獲利能力；可宣稱者全為相對比較。**
>
> **核心方法論發現：形成期的改良難以驗證，且難在設計、不在資料量。**
> 分組層的兩臂持有**不同的配對集合**（特異變異無從消去），
> 交易層的兩臂**共用配對**（差分即消噪）。兩者標準誤相差 **1.83 倍**，
> 等效於分組層需 **3.3 倍**樣本；欲將最小可偵測效果壓到 0.3pp，
> 分組層約需 **14.5 倍**樣本、逾三個半世紀的日資料。
> 對 0.3pp 的真實效果，分組層的檢定力僅 **11%**——
> **故該層的 null 對「分群是否較優」幾乎不具鑑別力。**
>
> **已知會再變動之處**（見 `thesis/07_附錄B_管線修正記錄.md` §B.5）：
> 交易端對沖口徑、分析層的基準格判定、兩份無上游指紋的快取，
> 以及兩支曾無產生器的一次性分析——皆已於 2026-09 修正並補為管線模組。


## 策略清單（`strategies_raw_all`，49 條，皆由 `cluster_formation.py` 中性組裝器動態產生，0-based 索引）
...
全部 49 條策略共用單一形成期模組 `strategies.formation.cluster_formation`（`formation_module`），
...
├── strategies/
│   ├── config.py                  # 全域參數、5×3 分組×排序 Grid 宣告式展開（49 條現役策略）、敏感性 OFAT 產生器
│   ├── db_utils.py                # SQLite 讀寫工具
│   ├── portfolio_manager.py       # 組合層級資金管理
│   ├── preprocess_equity.py       # 權益曲線前處理
│   ├── formation/
│   │   ├── cluster_formation.py       # ★ 中性組裝器：feature_mode × cluster_method × ranking_backend，49 條現役策略共用
