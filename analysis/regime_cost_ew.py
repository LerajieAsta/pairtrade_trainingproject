...
兩份輸出的差別很大，且方向相反，故兩者都要看：
  ...
  | | 最佳格 | 等權逐格對齊 |
  | regime 改善格數 | 12 改 / 8 劣 | **19 改 / 6 劣**（共 25 格） |
  | DL-THR 提高 break-even | 3/5 | **5/5** |
  | 成本餘裕 | +8.3 ~ +54.2 bps | **−18.9 ~ +34.4 bps** |

增益的兩項宣稱在等權口徑下更乾淨也更強（劣化的 6 格中有 5 格在 Turbulent，
即原本就獲利的那一檔）。成本餘裕的方向也如預期：最佳格
...
（論文 3.5.5 的口徑說明所引用者；先前此處寫「4.6.1、4.6.2 的口徑更正註腳」，
但論文中並無該註腳——兩節是直接改用等權口徑，未加註腳）。注意 `regime_cost_dsr_eval.run()`
