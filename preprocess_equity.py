...
if (results_dir / "quick_test").exists():
    target_datasets.append("quick_test")

if not target_datasets:
    print("❌ 未在 results/ 下找到 'current', 'full' 或 'quick_test' 資料夾，請確認回測路徑結構！")
    exit(1)
...
def parse_params_from_filename(filename):
    ...
    # 4. 匹配 全域動態停損 PSL
    match_psl = re.search(r'psl(\d+)', filename_lower)
    psl = f"{match_psl.group(1)}%" if match_psl and int(match_psl.group(1)) > 0 else "無"

    # 5. 匹配 配對產業上限 MSR
    match_msr = re.search(r'msr(\d+)', filename_lower)
    msr = f"{match_msr.group(1)}%" if match_msr and int(match_msr.group(1)) > 0 else "無"

    # 6. 匹配 動態停損 DSZ
    match_dsz = re.search(r'dsz(\d+)', filename_lower)
    dsz = match_dsz.group(1) if match_dsz and int(match_dsz.group(1)) > 0 else "無"

    # 7. 匹配 波動調節 Vol
    vol = "有" if "voladj" in filename_lower else "無"
    
    params_str = f"Top {top_n}, SL: {sl_pct}, ZWin: {zwin}, PSL: {psl}, MSR: {msr}, DSZ: {dsz}, VolAdj: {vol}"
...
for dataset in target_datasets:
    ...
    # 遍歷六大策略，挑選出該策略底下 Final Equity 最優 of CSV 進行指標計算與資產曲線合併
