"""
DRL FQI 探索版 C — MLP 編碼器 ＋ 單網路 max 目標（去遞迴 + 去 Double-Q）
======================================================================
純探索、未預先登記，不是論文正式結果。補完 2x2 設計的最後一格：

                    Double-Q（v3 官方目標）   單網路 max（去 Double-Q）
    LSTM 編碼器      v3（官方）                v3s（drl_fqi_singleq_trading）
    MLP 編碼器       v3m（drl_fqi_mlp_trading） v3ms（本檔）

其餘（獎勵、回測會計、訓練預算）逐字相同。
...
class MLPQNet(nn.Module):
    """攤平視窗 + MLP 市場編碼器（無遞迴）＋ 持倉條件 Q-head（與 v3m 相同）。"""
...
class Trading:
    """DRL FQI（MLP 編碼器 + 單網路 max，無 Double-Q）Trading Strategy Interface"""
    ...
    def _fqi_sweeps(self, ag: dict, n_sweeps: int):
        """單網路 max 目標（無 Double-Q），編碼器為 MLP。"""
    ...
    def _get_agent(self, period_start: str, ticker_a: str, ticker_b: str, hedge_ratio: float,
        ...
        if ag is None:
            ...
            print(f"    [DRL-FQI-MLP-SINGLEQ] New {'GLOBAL walk-forward' if is_global else 'period-shared'} "
