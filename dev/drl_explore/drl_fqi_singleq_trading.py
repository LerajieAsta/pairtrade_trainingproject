"""
DRL FQI 探索版 B — 拿掉 Double-Q，改回單網路 max_a' 目標
======================================================================
純探索、未預先登記，不是論文正式結果。v3（`archive/trading/
drl_fqi_trading.py`）在每 5 個 sweep 用「online 網路選 a*、target 網路
估值」的 Double-Q 目標抑制高估偏差；本檔把這步改回教科書式的單網路
`max_a' Q_target(t+1, a')`，其餘（LSTM 編碼器、獎勵、回測會計）逐字相同，
用來檢驗「Double-Q 這個演算法選擇本身是否有貢獻」。
...
class LSTMQNet(nn.Module):
    """LSTM 市場編碼器 + 持倉條件 Q-head（與 v3 相同）。"""
...
class Trading:
    """DRL FQI（單網路 max 目標，無 Double-Q）Trading Strategy Interface"""
    ...
    def _fqi_sweeps(self, ag: dict, n_sweeps: int):
        """在累積的轉移集上執行 FQI（單網路 max 目標，無 Double-Q）。"""
        ...
        for sweep in range(n_sweeps):
            if sweep % 15 == 0:
                ...
                target.eval()
            if sweep % 5 == 0:
                # 單網路目標：直接用 target net 對 a' 取 max，不借 online 網路選 a*
                with torch.no_grad():
                    ...
                    for a_idx in range(3):
                        ...
                        q_next_cols.append(target.q_from_h(hn_t, oh).max(dim=1).values)
    ...
    def _get_agent(self, period_start: str, ticker_a: str, ticker_b: str, hedge_ratio: float,
        ...
        if ag is None:
            ...
            print(f"    [DRL-FQI-SINGLEQ] New {'GLOBAL walk-forward' if is_global else 'period-shared'} "
