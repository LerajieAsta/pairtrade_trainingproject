...
核心差異：
  - Formation：
      Step 1 → 擷取每支股票的多維特徵向量（預設 stats13：動量、波動率、自相關、統計矩；可切換 path 模式）
...
class Formation:
    ...
    def __init__(
        ...
        feature_mode: str = "stats13",         # "stats13" | "path"
    ):
    ...
    def _umap_reduce(self, X: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            # 當頻譜佈局 (spectral layout) 因樣本數太少或矩陣奇異而失敗時，退回隨機初始化模式以防當機
            reducer = umap.UMAP(
                n_components  = n_comp,
                n_neighbors   = n_neigh,
                min_dist      = self.umap_min_dist,
    ...
    def run(self) -> pd.DataFrame:
        ...
        for sector, sec_tickers in sorted(sector_to_tickers.items()):
            ...
            # Step 2：降維 (UMAP 或 PCA) - 針對單一產業規模調整降維維度
                ...
                if self.reduce_method == "pca":
                    ...
                    if n_neigh < 2:
                        ...
                        try:
                            ...
                            # 針對小樣本扇區，Spectral 初始化可能因圖結構問題報錯，此時降級採用隨機初始化
                            reducer = umap.UMAP(
                                n_components  = n_comp,
                                n_neighbors   = n_neigh,
                                min_dist      = self.umap_min_dist,
