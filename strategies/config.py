...
# ── 交易期長度對照：config 全程固定 126，從未變動過 ─────────────────────
# 2026-08-24 以候選池代理掃描 trading_window ∈ {42,63,126,189,252,378}
# （NOGRP-DTW 臂、282 個共同交易期、entry_z/exit_z 不變、DTW w=15 排序）。
#
# 收斂率隨窗長單調上升（0.372 → 0.851）——延長交易期確實能把期末強平轉成
# 收斂。但每 21 日的組合報酬 = capture × 21/W（窗長加倍則同時持倉加倍、
# 每組資金減半），故除以稀釋因子後在 W=63 見頂：
#     W=42 0.00200 ／ 63 0.00300 ／ 126 0.00239 ／ 189 0.00186
#     ／ 252 0.00157 ／ 378 0.00118        （top-1，每單位資本捕獲）
# W=63 的後半期（2014 後）比值亦為全表最高（0.160 對現行 126 的 0.113），
# 且前後半皆正。
#
# ⚠ 三項保留：
#   1. 兩條臂的最優點不一致——GICS-SSD 臂上峰值在 W=252 而非 63，且六個
#      窗長的後半期全為負。最優點可能是雜訊而非結構。
#   2. 代理答不出 Sharpe：長窗雖稀釋每筆資金，卻同時持有更多部位（重疊期
#      6 對 3），分散效果會壓低組合波動，這一項代理算不出來。
#   3. 形成期必須一起跑，不能重用 126 日的配對——交易端讀的是形成期存下的
#      Trade_End（run_trading.py:277），沿用會變成「槽位開 3 期、實際持倉
#      6 期」的資金超額配置。
#
# ── 實測結果（2026-08-24，形成期 298 期 + 交易端 15 格）──────────────
# **假說被否證。** 同期比較（2001-01-02 ~ 2025-12-31）TW63 僅 1/15 格勝出、
# 中位差 −0.1389；Top1_SL0 為 0.392 對基準 0.468，DSR(N=46) 0.636 對 0.765。
#
# 代理的預測部分正確、部分失準，值得記錄：
#   對的——逐日報酬差年化 +0.572%，W=63 確實賺得多（與代理的每單位資本
#         捕獲 +24% 方向一致）。
#   錯的——Sharpe 反而更低。上方保留事項第 2 點應驗：重疊期由 6 降為 3，
#         分散不足使年化波動由 7.48% 升至 10.40%，抵銷有餘。
#
# 後半期表面上較佳（0.348 對 0.305）但不成立：Newey-West(21) 於 2014 後
# t=+0.640、p=0.5222，逐年僅 5/12 較佳；且拆解後是槓桿效果——後半年化報酬
# 1.71% 對 1.08%（1.58x）、波動 4.92% 對 3.56%（1.38x）。兩者的後半年化
# 皆低於 2% 無風險利率。
#
# 保留本條目的理由與 FW504 相同：它是一個有明確機制解釋的否證，不是可用策略。
# trading_window 這一維至此測畢，現行的 126 無須更動。
_p_tw63 = {**base_params, **_GRID_COMMON,
           "cluster_method": "none", "ranking_backend": "dtw",
           "ignore_ols_alpha": True,
           "trading_window": 63}
_grid_entries.append({
    "name":             "Grid NOGRP-DTW-TW63",
    "formation_module": "strategies.formation.cluster_formation",
    "trading_module":   "strategies.trading.zscore_trading",
    "sub_dir":          "Grid_NOGRP_DTW_TW63",
    "db_method":        "Grid (NOGRP-DTW-TW63)",
    "trade_method":     "Z-Score",
    "params":           _p_tw63,
})
